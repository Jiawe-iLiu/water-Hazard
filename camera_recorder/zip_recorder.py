...
def work(id, ImageQueue, ImageFilePattern, ImageCounter, TimeStampFile):
    tsFile = open(TimeStampFile, 'w')
    while True:
        Data = ImageQueue.get()
        if Data is None:
            break
        ...
        cv2.imwrite(ImageFilePattern % Index, Frame)
        tsFile.write('%f\n' % timeStamp)
        logging.debug("%d task: save image %d, queue length %d" %
                      (id, Index, ImageQueue.qsize()))
...
class ZipRecorder:
    ...
    '''
    def __init__(self, DeviceIndex=1, Resolution='HD', OutputFile='video.zip',
        ...
        if '#date' in self.OutputFile:
            self.OutputFile = \
                self.OutputFile.replace('#date', today.strftime("%d-%m-%Y"))
        if '#time' in self.OutputFile:
            self.OutputFile = \
                self.OutputFile.replace('#time', today.strftime("%H-%M-%S"))
    ...
    def start(self):
        ...
        if self.Camera.isOpened():
            ...
            self.workers = [Process(target=work, args=(i, self.ImageQueue, self.ImageFilePattern, self.ImageCounter, self.TimeStampFile))
                            for i in xrange(self.NUMBER_OF_PROCESSES)]
            for w in self.workers:
                w.start()

            serve(self.ImageQueue, self.Camera)
